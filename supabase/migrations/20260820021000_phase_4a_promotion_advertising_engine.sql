...
create table if not exists public.marketing_campaigns (
  id uuid primary key default gen_random_uuid(),
  campaign_id text not null unique default next_universal_id('MARKETING_CAMPAIGN'),
...
create table if not exists public.sponsored_listings (
  id uuid primary key default gen_random_uuid(),
  sponsored_listing_id text not null unique default next_universal_id('SPONSORED_LISTING'),
  ...
  status text not null default 'draft' check (status in ('draft','pending_review','approved','scheduled','active','paused','completed','cancelled','expired','suspended')),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
...
create table if not exists public.promotion_transactions (
  id uuid primary key default gen_random_uuid(),
  promotion_transaction_id text not null unique default next_universal_id('PROMOTION_TRANSACTION'),
...
create table if not exists public.promotional_banners (
  id uuid primary key default gen_random_uuid(),
  banner_id text not null unique default next_universal_id('PROMOTIONAL_BANNER'),
...
begin
  ...
  select * into c from public.marketing_campaigns where id=p_campaign_id for update;
  if c.id is null then raise exception 'campaign_not_found'; end if;
  if p_billable and not public.can_administer(auth.uid()) then raise exception 'billable_events_require_server_authority'; end if;
  if p_event_type not in ('promotion_impression','promotion_click','promotion_conversion') and not public.can_administer(auth.uid()) and c.advertiser_user_id<>auth.uid() then raise exception 'not_authorized'; end if;
...
returns setof public.promotional_banners language sql stable security definer set search_path=''
...
create policy marketing_campaigns_read on public.marketing_campaigns for select using(advertiser_user_id=auth.uid() or public.can_administer(auth.uid()) or status in('approved','scheduled','active','completed'));
create policy marketing_campaigns_insert on public.marketing_campaigns for insert with check(advertiser_user_id=auth.uid() and not exists(select 1 from public.profiles p where p.id=auth.uid() and p.account_status in('banned','suspended')));
create policy marketing_campaigns_update on public.marketing_campaigns for update using(advertiser_user_id=auth.uid() or public.can_administer(auth.uid())) with check(advertiser_user_id=auth.uid() or public.can_administer(auth.uid()));
create policy sponsored_listings_read on public.sponsored_listings for select using(seller_user_id=auth.uid() or public.can_administer(auth.uid()) or status in('approved','scheduled','active','completed'));
create policy sponsored_listings_write on public.sponsored_listings for all using(seller_user_id=auth.uid() or public.can_administer(auth.uid())) with check(seller_user_id=auth.uid() or public.can_administer(auth.uid()));
create policy promotion_transactions_read on public.promotion_transactions for select using(advertiser_user_id=auth.uid() or public.can_administer(auth.uid()));
create policy promotion_events_read on public.promotion_events for select using(user_id=auth.uid() or exists(select 1 from public.marketing_campaigns c where c.id=campaign_id and c.advertiser_user_id=auth.uid()) or public.can_administer(auth.uid()));
create policy promotion_daily_spend_read on public.promotion_daily_spend for select using(exists(select 1 from public.marketing_campaigns c where c.id=campaign_id and c.advertiser_user_id=auth.uid()) or public.can_administer(auth.uid()));
create policy promotion_statistics_read on public.promotion_statistics for select using(exists(select 1 from public.marketing_campaigns c where c.id=campaign_id and c.advertiser_user_id=auth.uid()) or public.can_administer(auth.uid()));
create policy promotional_banners_read on public.promotional_banners for select using((status in('active','scheduled') and (starts_at is null or starts_at<=now()) and (ends_at is null or ends_at>now())) or advertiser_user_id=auth.uid() or public.can_administer(auth.uid()));
create policy promotional_banners_write on public.promotional_banners for all using(advertiser_user_id=auth.uid() or public.can_administer(auth.uid())) with check(advertiser_user_id=auth.uid() or public.can_administer(auth.uid()));
create policy banner_links_read on public.banner_links for select using(exists(select 1 from public.promotional_banners b where b.id=banner_id and (b.status in('active','scheduled') or b.advertiser_user_id=auth.uid() or public.can_administer(auth.uid()))));
...
create policy banner_analytics_read on public.banner_analytics for select using(exists(select 1 from public.promotional_banners b where b.id=banner_id and b.advertiser_user_id=auth.uid()) or public.can_administer(auth.uid()));

-- Seed disabled pricing templates only; Super Admin controls real prices and activation.
insert into public.promotion_pricing(pricing_key,promotion_type,pricing_model,unit_price,enabled,config) values
('sponsored_listing_cpm','sponsored_listing','CPM',0,false,'{}'),
('sponsored_listing_cpc','sponsored_listing','CPC',0,false,'{}'),
('banner_cpm','banner','CPM',0,false,'{}')

...
export function NotificationCard({ notification, onReadChange }: { notification: Notification; onReadChange: (id: string, read: boolean) => void }) {
  ...
  const main = (
    <span className="flex min-w-0 flex-1 items-start gap-3">
      <span className={`grid h-11 w-11 shrink-0 place-items-center rounded-2xl border border-[var(--border)] bg-[var(--surface)] ${notification.is_read ? "opacity-60" : ""}`} aria-hidden="true"><Icon size={18} strokeWidth={notification.is_read ? 1.8 : 2.1} /></span>
  ...
  return <div className={className}>
    {destination ? <Link href={destination} onClick={() => { if (!notification.is_read) onReadChange(notification.id, true); }} className="min-w-0 flex-1 rounded-xl outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus)]/40">{main}</Link> : main}
    <span className="flex shrink-0 items-center gap-1.5 self-center">
      {destination && <ExternalLink size={15} className="text-[var(--muted)]" aria-hidden="true" />}
      <button type="button" onClick={() => onReadChange(notification.id, !notification.is_read)} className="rounded-lg p-2 text-[var(--muted)] hover:bg-[var(--surface-muted)] hover:text-[var(--foreground)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus)]/40" aria-label={notification.is_read ? "Mark notification as unread" : "Mark notification as read"}>{notification.is_read ? <Bell size={15} /> : <Check size={15} />}</button>
    </span>
  </div>;
}

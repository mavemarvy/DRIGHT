...
import { ArrowRight, BarChart3, DollarSign, FileText, Megaphone, ShieldCheck, ShoppingCart, Users } from "lucide-react";
...
export default function AdminHomePage() {
  ...
  return <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8"><div className="rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-6 sm:p-8"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-widest text-[var(--muted)]">DRIGHT Administration</p><h1 className="mt-2 text-3xl font-semibold">Admin Control Center</h1><p className="mt-2 max-w-2xl text-sm text-[var(--muted)]">Welcome, {name}. This is the separate administrative workspace for platform operations, trust, finance and configuration.</p></div><ShieldCheck className="hidden sm:block" size={38}/></div></div><section className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-4">{cards.map(({title,description,href,icon:Icon}) => <Link key={href} href={href} className="group rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 transition hover:-translate-y-0.5 hover:bg-[var(--background)]"><div className="flex items-center justify-between"><Icon size={21}/><ArrowRight size={17} className="text-[var(--muted)] transition group-hover:translate-x-1"/></div><h2 className="mt-6 font-semibold">{title}</h2><p className="mt-2 text-sm text-[var(--muted)]">{description}</p></Link>)}</section><section className="mt-6 grid gap-4 sm:grid-cols-3"><div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5"><ShoppingCart size={20}/><p className="mt-4 text-sm font-semibold">Commerce</p><p className="mt-1 text-xs text-[var(--muted)]">Orders, listings and fulfillment controls will live here as the admin workspace expands.</p></div><div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5"><Users size={20}/><p className="mt-4 text-sm font-semibold">Users & Trust</p><p className="mt-1 text-xs text-[var(--muted)]">Identity, moderation, reports and account operations will be centralized here.</p></div><div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5"><BarChart3 size={20}/><p className="mt-4 text-sm font-semibold">Analytics</p><p className="mt-1 text-xs text-[var(--muted)]">Platform intelligence and operational metrics will use this dedicated control space.</p></div></section></main>;
}

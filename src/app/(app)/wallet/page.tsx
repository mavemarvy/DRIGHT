...
export default function WalletPage() {
  ...
  return <div className="mx-auto max-w-6xl px-4 py-7 sm:px-6 lg:px-8">
    ...
    <section className="mt-6 grid gap-4 sm:grid-cols-3">
      {[{label:"Available",value:wallet?.available_balance||0,icon:WalletIcon},{label:"Pending payout",value:wallet?.pending_balance||0,icon:Clock3},{label:"Total balance",value:wallet?.total_balance||0,icon:Banknote}].map(({label,value,icon:Icon}) => <article key={label} className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 shadow-sm"><div className="flex items-center justify-between"><p className="text-xs font-semibold uppercase tracking-wide text-[var(--muted)]">{label}</p><span className="grid h-9 w-9 place-items-center rounded-xl bg-[var(--background)]"><Icon size={17}/></span></div><p className="mt-4 text-2xl font-bold">{loading ? "—" : money(Number(value))}</p>{label === "Pending payout" && <p className="mt-1 text-xs text-[var(--muted)]">{wallet?.pending_payout_count || 0} active request(s)</p>}</article>)}
    </section>

    <div className="mt-5 grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
      <section className="rounded-[28px] border border-[var(--border)] bg-[var(--surface)] p-6 shadow-sm sm:p-7">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-sm text-[var(--muted)]">Selected wallet</p><p className="mt-1 text-3xl font-bold">{loading ? "—" : wallet?.currency_code || "USD"}</p><p className="mt-2 text-xs text-[var(--muted)]">Wallet ID: {wallet?.public_wallet_id || "—"}</p></div>{wallets.length > 1 && <select value={selectedWallet} onChange={e => setSelectedWallet(e.target.value)} className="rounded-xl border border-[var(--border)] bg-[var(--background)] px-3 py-2.5 text-sm">{wallets.map(w => <option key={w.wallet_id} value={w.wallet_id}>{w.currency_code}</option>)}</select>}</div>
        <div className="mt-5 flex flex-wrap gap-2"><button onClick={copyWalletId} className="inline-flex items-center gap-2 rounded-xl border border-[var(--border)] px-3 py-2 text-xs font-semibold"><Copy size={14}/> Copy wallet ID</button><Link href="/settings" className="inline-flex items-center gap-2 rounded-xl border border-[var(--border)] px-3 py-2 text-xs font-semibold"><CreditCard size={14}/> Payment settings</Link></div>
        <div className="mt-6 rounded-2xl border border-dashed border-[var(--border)] bg-[var(--background)] p-5"><div className="flex gap-3"><ArrowUpFromLine size={20} className="mt-0.5 shrink-0"/><div><p className="font-semibold">Fund wallet</p><p className="mt-1 text-sm leading-6 text-[var(--muted)]">Direct wallet funding is not enabled by the current DRIGHT payment infrastructure. Do not send money to a manual account or enter payment credentials here. Purchases continue through the secure checkout flow.</p><Link href="/marketplace" className="mt-3 inline-flex items-center gap-2 text-sm font-semibold text-[var(--primary)]">Browse marketplace <ExternalLink size={14}/></Link></div></div></div>
      </section>

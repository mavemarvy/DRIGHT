...
type CartRow = {
  ...
  marketplace_items: { id: string; title: string; item_type: string | null; price: number | null; currency: string | null } | null;
};
...
export default function CartPage() {
  ...
  async function loadCart() {
    ...
    setCartId(cart.id);
    const { data, error: itemError } = await supabase.from("cart_items").select("id,quantity,unit_price,currency,marketplace_item_id,marketplace_items(id,title,item_type,price,currency)").eq("cart_id", cart.id).order("created_at", { ascending: false });
  ...
  const currency = rows[0]?.currency || rows[0]?.marketplace_items?.currency || "USD";
  const mixedCurrencies = rows.some(row => (row.currency || row.marketplace_items?.currency || "USD") !== currency);

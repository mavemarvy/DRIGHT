...
type Recommendation = { id: string; public_id: string; title: string; item_type: string; price: number | null; currency_code: string | null; final_score: number; recommendation_reason: string; algorithm_version: string };
type Similar = { id: string; public_id: string; title: string; item_type: string; price: number | null; currency_code: string | null; similarity_score: number; recommendation_reason: string };
...
export default function ListingDetailPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [{ data: similarData }, { data: recommendationData }] = await Promise.all([
        supabase.rpc("get_similar_marketplace_items", { p_item_id: id, p_limit: 6 }),
        supabase.rpc("get_marketplace_recommendations", { p_limit: 6, p_exclude_item_id: id }),
      ]);
      setSimilar((similarData ?? []) as Similar[]);
      ...
      const { data: favorite } = await supabase.from("marketplace_item_favorites").select("item_id").eq("user_id", user.id).eq("item_id", id).maybeSingle();
      setSaved(Boolean(favorite));
      await supabase.rpc("record_discovery_event", { p_entity_id: id, p_event_type: "open", p_source: "marketplace", p_metadata: { public_id: data.public_id } });
      if (recommendationData?.length) {
        await Promise.all((recommendationData as Recommendation[]).map((entry, index) =>
          supabase.rpc("record_discovery_event", { p_entity_id: entry.id, p_event_type: "impression", p_source: "recommendation", p_position: index + 1, p_metadata: { algorithm_version: entry.algorithm_version, source_item_id: id } })
        ));
      }
    })();
  }, [id, supabase]);
  ...
  return <div className="mx-auto max-w-6xl px-4 py-7 sm:px-6 lg:px-8">
    ...
    {(similar.length > 0 || recommendations.length > 0) && <section className="mt-10 space-y-8">
      {similar.length > 0 && <div><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-widest text-[var(--muted)]">Similarity engine</p><h2 className="mt-1 text-2xl font-semibold">Similar listings</h2></div><Sparkles size={20} className="text-[var(--muted)]" /></div><div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{similar.map((entry) => <RecommendationCard key={entry.id} id={entry.id} publicId={entry.public_id} title={entry.title} itemType={entry.item_type} price={entry.price} currency={entry.currency_code} reason={entry.recommendation_reason} />)}</div></div>}
      {recommendations.length > 0 && <div><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-widest text-[var(--muted)]">Personalized discovery</p><h2 className="mt-1 text-2xl font-semibold">Recommended for you</h2></div><Sparkles size={20} className="text-[var(--muted)]" /></div><div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{recommendations.map((entry) => <RecommendationCard key={entry.id} id={entry.id} publicId={entry.public_id} title={entry.title} itemType={entry.item_type} price={entry.price} currency={entry.currency_code} reason={entry.recommendation_reason} />)}</div></div>}
    </section>}
  </div>;
}

function RecommendationCard({ id, publicId, title, itemType, price, currency, reason }: { id: string; publicId: string; title: string; itemType: string; price: number | null; currency: string | null; reason: string }) {
  const displayPrice = price == null ? "Contact seller" : `${currency || "USD"} ${Number(price).toLocaleString()}`;
  return <Link href={`/marketplace/${id}`} className="group rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 transition hover:-translate-y-0.5 hover:border-[var(--primary)]"><div className="flex items-start justify-between gap-3"><span className="rounded-full bg-[var(--background)] px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-[var(--muted)]">{labelFor(itemType)}</span><span className="font-mono text-[10px] text-[var(--muted)]">{publicId}</span></div><h3 className="mt-4 line-clamp-2 text-base font-semibold group-hover:underline">{title}</h3><p className="mt-2 text-sm font-semibold">{displayPrice}</p><p className="mt-2 text-xs leading-5 text-[var(--muted)]">{reason}</p></Link>;
}
